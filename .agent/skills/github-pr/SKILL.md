...
- En la extension del piloto, `github-pr` lidera cuando el problema principal es preparar, describir, revisar o abrir Pull Requests con `gh`.
- Si la rama aun necesita cambios de codigo, quedar como skill de apoyo y dejar que la skill tecnica correspondiente cierre primero la implementacion.
- Si la tarea es transversal y ya esta implementada, coordinar con `orquestador-flujo` para resumir cambios y evidencia antes de crear el PR.

## Contrato minimo de handoff

- Al recibir o delegar un handoff, exigir: `objetivo`, `skill-lider`, `usuario/tarea`, `restricciones`, `archivos/rutas`, `defaults a evitar` y `salida esperada`.
- Si `github-pr` lidera, conserva la autoridad sobre narrativa del PR, estructura de descripcion, convencion del titulo y uso correcto de `gh`.
