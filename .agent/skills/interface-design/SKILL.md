...
- Choose `interface-design` when the main challenge is product structure, navigation, workflows, dense information, data display, or operational clarity.
- Choose `frontend-design` when the main challenge is brand expression, storytelling, atmosphere, promotional surfaces, or making the frontend feel visually unforgettable.
- If a task mixes both, use `interface-design` for the shell, hierarchy, navigation, and task flow; use `frontend-design` for hero sections, branded moments, and high-expression surfaces.

## Trigger Signals

Use this skill when the user asks for things like:
- dashboards, admin panels, backoffice screens, settings pages, internal tools, analytics, or SaaS product areas
- better information hierarchy, denser but clearer layouts, improved navigation, or more usable operational screens
- tables, filters, forms, metrics, workflows, sidebars, tabs, command surfaces, or data-heavy UI

## Guardrails

- Prioritize task completion, hierarchy, and orientation before expressive styling.
- Preserve room for personality, but never sacrifice operational clarity for visual theater.
- When the existing system is already branded, reinforce that language instead of introducing marketing-style flourishes.

## Regla del repo: fuente visual obligatoria

- Si existe `docs/esenciales/diseno/`, leer esa carpeta antes de redisenar dashboards, shells, tablas o modulos operativos.
- Si NO existe base visual formal, no arrastrar sistemas heredados de otros repos; primero definir una direccion visual minima coherente con `docs/esenciales/`.
- Si el problema mezcla claridad operativa y expresion visual, reforzar la identidad vigente del repo sin romper la usabilidad.
